...
# ---- ab hier Umwandlung eingelesenes Mesh -> JuAFEM.mesh
    
    # element type
    # vorerst nur ein Elementtyp zugelassen
    oneelmtype = true
    for i in 2:length(elmcount)
    ...
    if(gmsh_elmtype_key == 3) # 2d-Quadrilateral elements
        nodes = Vector{JuAFEM.Node{2,Float64}}(undef, nonodes)
        for i in 1:nonodes
            nodes[i] = JuAFEM.Node(Vec{2}(coord[1:2,i]))
        end
    elseif(gmsh_elmtype_key == 4) # 3d-Tetrahedron elements
        nodes = Vector{JuAFEM.Node{3,Float64}}(undef, nonodes)
        for i in 1:nonodes
            nodes[i] = JuAFEM.Node(Vec{3}(coord[:,i]))
        end
    ...
    #    cells = Vector{JuAFEM.Line}(undef, length(elms))
    #elseif(gmsh_elmtype_key==2)
    #    cells = Vector{JuAFEM.Triangle}(undef, length(elms))
    #elseif(gmsh_elmtype_key==3)
    if(gmsh_elmtype_key==3)
        cells = Vector{JuAFEM.Quadrilateral}(undef, length(elms))
        for (i, elm) in enumerate(elms)
            s = split(elm)
            cellnodes = (parse(Int, s[2]), parse(Int, s[3]), parse(Int, s[4]), parse(Int, s[5]))
            cells[i] = JuAFEM.Quadrilateral(cellnodes)
        end
    elseif(gmsh_elmtype_key==4)
        cells = Vector{JuAFEM.Tetrahedron}(undef, length(elms))
        for (i, elm) in enumerate(elms)
            s = split(elm)
            cellnodes = (parse(Int, s[2]), parse(Int, s[3]), parse(Int, s[4]), parse(Int, s[5]))
            cells[i] = JuAFEM.Tetrahedron(cellnodes)
        end
    elseif(gmsh_elmtype_key==5)
        cells = Vector{JuAFEM.Hexahedron}(undef, length(elms))
        for (i, elm) in enumerate(elms)
            s = split(elm)
            ...
            cells[i] = JuAFEM.Hexahedron(cellnodes)
        end
    end
...
#----------------------------
    ...
    if(gmsh_elmtype_key==3) #2d
        for i in 1:length(physname) # loop physical names
            name = physname[i]
            if(physdim[name] == 2)
                number = physnum[name]
                cellsofset = Set{Int}()
                for j in 1:length(physentities) # loop entities
    ...
    return JuAFEM.Grid(cells, nodes; cellsets = cellsets) 
end